...
const ADMIN_EMAIL = 'sunilkumarsingh817@gmail.com';
...
export const AppStateProvider = ({ children }: { children: ReactNode }) => {
  ...
  const logOperationAttempt = useCallback((
    operation: string, 
    path: string, 
    ...
    console.log(
      ...
      { 
        ...
        actingUserEmailVerified: actingUser?.emailVerified, // This comes from FirebaseUser object
        isAdminClientState: isAdmin, // This is the context's isAdmin state
        details, 
        timestamp: new Date().toISOString() 
      }
    );
  }, [isAdmin]); // isAdmin is a dependency for logOperationAttempt
  ...
  const ensureUserProfileExists = useCallback(async (user: FirebaseUser, nameFromSignup?: string) => {
    ...
    try {
      ...
      if (!userProfileSnap.exists()) {
        ...
        if (user.displayName !== effectiveName) { 
        ...
        if (user.displayName !== (updates.name || existingProfileData.name) && (updates.name || existingProfileData.name)) { 
  ...
  useEffect(() => {
    ...
    const unsubscribe = onAuthStateChanged(auth, async (user) => {
      if (user) {
        ...
        const newIsAdmin = isUserAdminEmail; // Email verification check removed for admin as per request
  ...
  }, [ensureUserProfileExists]); // logOperationAttempt removed as it's now a dependency of ensureUserProfileExists

  const loadAdminData = useCallback(async () => {
    const currentAdminUser = auth.currentUser; // Use current user from auth directly for admin check
    if (!currentAdminUser || currentAdminUser.email !== ADMIN_EMAIL) {
     ...
     console.log(
        ...
        ADMIN_EMAIL constant: ${ADMIN_EMAIL},
        Client-side isAdmin flag (from context): ${isAdmin}`, // This context `isAdmin` might be slightly delayed
  ...
  }, [toast, isAdmin, logOperationAttempt]); // isAdmin included to re-evaluate if it changes

  const loadUserSpecificData = useCallback(async (userIdToLoad: string) => {
    ...
    try {
      ...
      // If admin viewing another user's data, schools are fetched by fetchSchoolsForUser for admin pages
      ...
      // If admin viewing another user's data, students are fetched by fetchStudentsForClass for admin pages
    } catch (error) {
  ...
  useEffect(() => {
    const performDataLoad = async () => {
        if (!currentUser) { // Only proceed if currentUser is explicitly null (logged out) or an actual user object
            if(currentUser === null) { // User is definitively logged out
                ...
                setUserProfiles([]);
                setIsLoading(false);
            } else { // currentUser is undefined, auth state still initializing
                 console.log("%cAppStateContext: Data load effect SKIPPED (currentUser is undefined - auth state still initializing).", "color: gray;");
                 setIsLoading(true);
            }
            return;
        }
        ...
        setIsLoading(true);
        if (isAdmin) { 
            console.log("%cAppStateContext: ADMIN detected by client. Attempting to load ALL user profiles AND admin's own specific data.", "color: magenta; font-weight: bold;");
            await loadAdminData(); 
            await loadUserSpecificData(currentUser.uid); 
        } else {
            console.log("%cAppStateContext: REGULAR USER detected. Clearing any existing user profiles (if any) and loading THEIR specific data.", "color: cyan; font-weight: bold;");
            setUserProfiles([]); 
            await loadUserSpecificData(currentUser.uid);
        }
        setIsLoading(false);
  ...
  }, [currentUser, isAdmin, loadAdminData, loadUserSpecificData]);


  const addSchool = async (schoolData: Omit<School, 'id' | 'userId' | 'appwriteBucketId'>, targetUserId?: string) => {
    ...
    if (effectiveUserId === currentAuthUser.uid && !isAdmin) { // If a regular user adds their own school
      setSchools(prev => [...prev, newSchool]); 
    } else if (isAdmin && effectiveUserId === currentAuthUser.uid) { // If admin adds for themselves
      setSchools(prev => [...prev, newSchool]);
    }
    // If admin adds for another user, their list will update on next fetch in admin panel.
    toast({ title: "School Added", description: `School "${newSchool.name}" added for user ${effectiveUserId}.`});
  };

  const updateSchool = async (updatedSchoolData: Omit<School, 'userId' | 'appwriteBucketId'>, targetUserId?: string) => {
    const currentAuthUser = auth.currentUser;
    ...
    // Ensure appwriteBucketId is not part of the data being saved if it was just for type consistency
    ...
    if (effectiveUserId === currentAuthUser.uid && !isAdmin) {
    ...
    } else if (isAdmin && effectiveUserId === currentAuthUser.uid) {
      setSchools(prev => prev.map(s => s.id === updatedSchoolData.id ? { ...s, ...dataToSave, id: updatedSchoolData.id, userId: effectiveUserId } as School : s));
    }
    toast({ title: "School Updated", description: `School "${updatedSchoolData.name}" updated for user ${effectiveUserId}.`});
  };
  
  const updateSchoolClassNames = async (schoolId: string, classNames: string[], targetUserId?: string) => {
    const currentAuthUser = auth.currentUser;
  ...
  const deleteSchool = async (schoolId: string, targetUserId?: string) => {
    const currentAuthUser = auth.currentUser;
    ...
    for (const studentDoc of studentsSnapshot.docs) {
      ...
      if (studentData.photoAppwriteId) { 
        try {
          ...
          await deleteFileFromAppwriteStorage(studentData.photoAppwriteId);
        } catch (appwriteError) {
  ...
  const addStudent = async (
    studentData: Omit<Student, 'id' | 'userId' | 'photoAppwriteId'>, 
    photoFile: File | null, 
    ...
    if (photoFile) {
        ...
        studentDataForFirestore.photoAppwriteId = appwritePhotoId;
    } else {
        delete studentDataForFirestore.photoAppwriteId; // Ensure field is omitted if no photo
    }
    ...
    if (effectiveUserId === currentAuthUser.uid) { // Update local state only if current user is the one adding for themselves
  ...
  const updateStudent = async (
    updatedStudentDataWithId: Omit<Student, 'userId' | 'photoAppwriteId'> & { id: string, photoAppwriteId?: string }, 
    photoFile: File | null, 
    schoolNameForPath: string, 
    classNameForPath: string, 
    oldPhotoAppwriteIdToDelete?: string, 
    targetUserId?: string
  ) => {
    const currentAuthUser = auth.currentUser;
    ...
    if (photoFile) { 
        ...
        finalDataForFirestore.photoAppwriteId = newPhotoAppwriteId;
        if (oldPhotoAppwriteIdToDelete && oldPhotoAppwriteIdToDelete !== newPhotoAppwriteId) {
            ...
            await deleteFileFromAppwriteStorage(oldPhotoAppwriteIdToDelete);
        }
    } else if (oldPhotoAppwriteIdToDelete && !studentDataToSave.photoAppwriteId) { // Photo explicitly removed
        ...
        await deleteFileFromAppwriteStorage(oldPhotoAppwriteIdToDelete);
        finalDataForFirestore.photoAppwriteId = deleteField() as any; // Use deleteField to remove from Firestore
    } else if (studentDataToSave.photoAppwriteId) { // Existing photo ID kept
        finalDataForFirestore.photoAppwriteId = studentDataToSave.photoAppwriteId;
    } else {
        delete finalDataForFirestore.photoAppwriteId; // Ensure field is omitted if no photo involved
    }
    ...
    if (effectiveUserId === currentAuthUser.uid) { // Update local state only if current user is the one updating for themselves
  ...
  const deleteStudent = async (studentId: string, targetUserId?: string) => {
    const currentAuthUser = auth.currentUser;
    ...
    if (effectiveUserId === currentAuthUser.uid) { // Update local state only if current user is the one deleting for themselves
  ...
  const fetchSchoolsForUser = async (userId: string): Promise<School[]> => {
    const currentAuthUser = auth.currentUser;
  ...
  const fetchStudentsForClass = async (userId: string, schoolId: string, className: string): Promise<Student[]> => {
    const currentAuthUser = auth.currentUser;
  ...
  const fetchSchoolByIdForUser = async (userId: string, schoolId: string): Promise<School | null> => {
    const currentAuthUser = auth.currentUser;
